CounosCoin Core version *0.15.1* is now available from:

  <https://download.counoscoin.org/counoscoin-0.15.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/counoscoin-dev>
...
installer (on Windows) or just copy over `/Applications/CounosCoin-Qt` (on Mac)
or `counoscoind`/`counoscoin-qt` (on Linux).
...
CounosCoin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

CounosCoin Core should also work on most other Unix-like systems but is not
...
A number of changes to the way CounosCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 CCA). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 CCA) in the next release.
